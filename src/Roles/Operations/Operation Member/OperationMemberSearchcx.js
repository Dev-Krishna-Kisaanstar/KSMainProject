  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/advisory-member/search-customer/${trimmedMobileNumber}`,{ withCredentials: true }
      );
      ...
      if (response.data.message === 'Customer fetched Successfully') {
        ...
        setCustomer(null);
        navigate('/AdvisorAdminAddCx', { state: { mobileNumber } });
      }
    } catch (error) {
      if (error.response) {
