...
const StyledPaper = styled(Paper)(({ theme }) => ({
    backgroundColor: 'rgba(30, 30, 47, 0.8)', // Glass effect with transparency
    backdropFilter: 'blur(10px)', // Apply blur to the background
    color: '#fff',
    padding: theme.spacing(3),
    borderRadius: '10px', // Optional: Add rounded corners for a softer look
    '& .MuiTextField-root': {
    ...
    '& .MuiOutlinedInput-root .MuiOutlinedInput-notchedOutline': { // Style the input border
        borderColor: 'rgba(255, 255, 255, 0.3)', // Faint white border
    },
    '& .MuiInputLabel-root': { // Style the input label
        color: 'rgba(255, 255, 255, 0.7)', // Faint white label
    },
    '& .MuiInputLabel-root.Mui-focused': { // Style the input label when focused
        color: '#fff', // Solid white label when focused
    },
     '& .MuiFormHelperText-root': {  // Helper text style
        color: '#fff',
    },
    '& .MuiButton-root': {
        marginRight: theme.spacing(1),
        '&:hover': {
            backgroundColor: 'rgba(8, 171, 86, 0.7)', // Green hover effect with transparency
        },
    },
}));

function VendorMemberRegistrationForm() {
            ...
            <StyledPaper elevation={3} className="container my-5">
                ...
                <Stepper activeStep={currentStep} alternativeLabel className="mb-4">
                    {STEPS.map((step, index) => (
                        <Step key={index}>
                            <StepLabel>
                                <Typography variant="body2" style={{ fontWeight: currentStep === index ? 'bold' : 'normal', color: currentStep === index ? '#1976d2' : '#fff' }}>
                                    {step.label}
                                </Typography>
                            </StepLabel>
                        </Step>
...
export default VendorMemberRegistrationForm;